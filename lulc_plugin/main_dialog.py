...
class MainDockWidget(QDockWidget):
    ...
    def _go_to_thresholds(self):
        if not credentials_are_saved():
            QMessageBox.warning(
                ...
                "Please fill in your GEE/Drive settings first using the Settings button."
            )
            return
    ...
    def _get_drive_folder_id(self):
        from qgis.core import QgsSettings
        return QgsSettings().value("lulc_plugin/drive_folder_id", "")
    ...
    def _run_preview(self):
        ...
        self.task = ClassificationTask(
            ...
            self._get_key_path(), self._get_drive_folder_id(),
    ...
    def _on_task_finished(self, success, error, result_data):
        ...
        if self.task.mode == 'preview':
            ...
            shp_path = result_data
            layer = QgsVectorLayer(shp_path, "LULC Classification", "ogr")
            if layer.isValid():
                QgsProject.instance().addMapLayer(layer)
                QMessageBox.information(self, "Success", "Export complete and loaded into QGIS.")
            else:
                QMessageBox.critical(self, "Error", "Downloaded shapefile could not be loaded.")
